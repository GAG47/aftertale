...
func get_current_location_view() -> Dictionary:
	...
	return {
		...
		"view": {
			...
			"current_source_archetype_id": str(location.get("source_archetype_id", "")),
			"current_source_form_id": str(location.get("source_form_id", "")),
			...
			"current_gameplay_affordances": (location.get("gameplay_affordances", []) as Array).duplicate(),
			"current_narrative_affordances": (location.get("narrative_affordances", []) as Array).duplicate(),
...
func _runtime_edge_view(edge: Dictionary, target_location_id: String, locations_by_id: Dictionary, reverse_traversal: bool) -> Dictionary:
	...
	return {
		...
		"target_archetype_id": str(target.get("source_archetype_id", "")),
		"target_form_id": str(target.get("source_form_id", "")),
