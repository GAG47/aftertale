...
func check() -> ActionResult:
	...
	if location_root == null:
		return _failure("HarvestAction requires a location root.")

	var cell: Vector2i = target.get("cell", actor.grid_position) as Vector2i
	var failed_requirement: String = CropSystem.get_harvest_failure(actor, location_root, cell)
...
func execute() -> ActionResult:
	...
	return CropSystem.execute_harvest(actor, _get_location_root(), target.get("cell", actor.grid_position) as Vector2i)
