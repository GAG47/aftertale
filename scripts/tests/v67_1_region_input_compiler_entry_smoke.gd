...
func _assert_compile_entry_exposes_current_location_node_boundary() -> bool:
	...
	var result: Dictionary = compiler.compile_to_location_graph_result([_load_json(DEFAULT_REGION_INPUT_PATH)], EDGE_PROFILE_PATH, GRAPH_ID)
	if not bool(result.get("success", false)):
		_fail("v67.1 compiler entry did not reach the current LocationGraphSnapshot product: %s" % str(result.get("errors", [])))
		return false
	if (result.get("location_graph_snapshot", {}) as Dictionary).is_empty():
		_fail("v67.1 compiler entry did not include the valid LocationGraphSnapshot")
		return false
	return true
